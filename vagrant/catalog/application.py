...
def itemDescription(category_name, item_title):
    item = dbsession.query(Item).filter(and_(
                     Item.title == item_title,
                     Item.category.has(Category.name == category_name))).one()
    return render_template(
...
def editItem(category_name, item_title):
    ...
    currentItem = dbsession.query(Item).filter(and_(
                    Item.title == item_title,
                    Item.category.has(Category.name == category_name))).one()
    if currentItem.user_id != session['user_id']:
        ...
        output += 'Please add your own item in order to edit.");'
    ...
    if request.method == 'POST':
        ...
        if len(existing_items) >= 1:
            flash('Item exists, please pick a different title or category.')
            return render_template(
                                   "editItem.html",
                                   category_name=category_name,
            ...
            editedItem = dbsession.query(Item).filter(and_(
                    Item.title == item_title,
                    Item.category.has(Category.name == category_name))).one()
            ...
            dbsession.commit()
            return redirect('/catalog/{}/{}'.format(new_cat_name,
                                                    editedItem.title))
    else:
        return render_template(
                               "editItem.html",
...
def deleteItem(category_name, item_title):
    ...
    if currentItem.user_id != session['user_id']:
        ...
        output += 'Please add your own item in order to delete.");'
        output += ' window.location="/index";}</script>'
        output += '<body onload="myFunction()">'
        return output
    if request.method == 'POST':
        # In case there are two identical items,
        # use first() to at least delete one.
        # Avoid errors when using one()
        deletedItem = dbsession.query(Item).filter(and_(
            Item.title == item_title,
            Item.category.has(Category.name == category_name))).first()
